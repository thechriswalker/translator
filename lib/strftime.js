//forked from github.com/martinandert/counterpart
//I basically removed the ordinal, as they we english may need translating.
//instead the "o" code looks for an `ordinals` function passed in which should be localised
//Also I have made the format handling stricter.
"use strict";

function strftime(date, format, names, ordinals) {
  var timestamp = date.getTime();

  return format.replace(/%([-_0]?)(.)/g, function(match, modifier, code) {
    var padding = null;
    switch (modifier) {
      case "-": padding = "";  break;
    ...
    switch (code) {
      ...
      case "n": return "\n";
      case "o": return String(date.getDate()) + ordinals(date.getDate());
      ...
      case "r": return strftime(date, "%I:%M:%S %p", names);
      ...
      case "v": return strftime(date, "%e-%b-%Y",names);
      ...
      case "%": if(!modifier){ return "%"; }
        /* falls through */
      default:
        throw new Error("strftime invalid replacement in format: `"+match+"`");
    }
  });
}
